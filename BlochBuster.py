...
gif or mp4 files, which can be 3D plots of the magnetization vectors, plots of
transverse and longitudinal magnetization, or pulse sequence diagrams. Input
paramaters are provided by human readable configuration files. The animations
are made using ffmpeg.
"""
...
def applyPulseSeq(config, Meq, M0, w, T1, T2, pos0, v, D):
    ...
    for rep in range(
        ...
        for i, event in enumerate(config["events"]):
            ...
            M[firstFrame : lastFrame + 1] = integrate.odeint(
...
def loadGradfromFile(filename):
    ...
    with open(filename, "r") as f:
        ...
        if isinstance(grad, list) and len(grad) > 0 and isinstance(grad[0], Number):
            ...
            raise Exception(
                "Error reading gradient file {}. File must contain a yaml list of numbers.".format(
                    filename
                )
            )
...
def checkPulseSeq(config):
    ...
    for event in config["pulseSeq"]:
        ...
        if "spoil" in event:  # Spoiler event
            ...
            if "dur" not in event:
                raise Exception(
                    "All pulseSeq events except spoiler events must have a duration dur [msec]"
                )
        ...
        if "FA" in event or "B1" in event:  # RF-pulse event (possibly with gradient)
            ...
            if "FA" in event:
                if "B1" not in event:
                    event["B1"] = np.array(
                        [event["FA"] / (event["dur"] * 360 * gyro * 1e-6)]
                    )
                else:
                    event["B1"] = (
                        event["B1"] * event["FA"] / calcFA
    ...
    for event in config["pulseSeq"]:
        ...
        if len(arrLengths) > 0:  # arrays in event
            ...
            if len(set(arrLengths)) == 2 and 1 in set(arrLengths):
                ...
                raise Exception(
                    "If w1, Gx, Gy, Gz of an event are provided as lists, equal length is required"
                )
            for i, t in enumerate(
                np.linspace(
                    event["t"], event["t"] + event["dur"], arrLength, endpoint=False
                )
            ):
                    ...
                    if key in event:
                        if type(event[key]) is list:
                            if i < len(event[key]):
                                ...
                                raise Exception(
                                    "Length of {} does not match other event properties".format(
                                        key
                                    )
                                )
                        else:
                        ...
                        if key in ["Gx", "Gy", "Gz"]:
                            subEvent["{}text".format(key)] = "{}: {:2.0f} mT/m".format(
                                key, subEvent[key]
                            )
...
def getPrescribedTimeVector(config, nTR):
    ...
    t = 0
    dt = (
        1e3 / config["fps"] * config["speed"][0]["speed"]
    )  # Animation time resolution [msec]
    for event in speedEvents:
        kernelTime = np.concatenate(
            (kernelTime, np.arange(t, event["t"], dt)), axis=None
        )
        t = max(t, event["t"])
        if "speed" in event:
            dt = (
                1e3 / config["fps"] * event["speed"]
            )  # Animation time resolution [msec]
        if "FA" in event or "B1" in event:
            RFdt = min(
                dt, 1e3 / config["fps"] * config["maxRFspeed"]
            )  # Time resolution during RF [msec]
            kernelTime = np.concatenate(
                (kernelTime, np.arange(event["t"], event["t"] + event["dur"], RFdt)),
                axis=None,
            )
            t = event["t"] + event["dur"]
...
def setupPulseSeq(config):
    ...
    for i, event in enumerate(config["separatedPulseSeq"]):
        ...
        for stoppingEvent in [
            event
            for event in ongoingEvents[::-1]
            if roundEventTime(event["t"] + event["dur"]) <= nextEventTime
        ]:
            config["events"].append(dict(newEvent))
            newEvent = detachEvent(
                newEvent,
                stoppingEvent,
                roundEventTime(stoppingEvent["t"] + stoppingEvent["dur"]),
            )
            ongoingEvents.pop()
    config["events"].append(dict(newEvent))
    ...
    config["kernelClock"] = addEventsToTimeVector(
    ...
    for rep in range(
        ...
        config["t"] = np.concatenate(
            (config["t"], roundEventTime(config["kernelClock"] + rep * config["TR"])),
            axis=None,
        )
    config["t"] = np.concatenate(
        (config["t"], roundEventTime(config["nTR"] * config["TR"])), axis=None
...
def checkConfig(config):
    ...
    if "fps" not in config:
        config[
            "fps"
        ] = 15  # Frames per second in animation (<=15 should be supported by powepoint)
    ...
    if not "locations" in config:
        ...
        if isinstance(config["locations"], dict):
            for comp in iter(config["locations"]):
                config["locations"][comp] = arrangeLocations(
                    config["locations"][comp], config
                )
...
def spherical2cartesian(spherical):
    ...
    M = np.dot(
        np.dot(np.array([0, 0, r]), rotMatrix(np.radians(azim), 1)),
        rotMatrix(np.radians(polar), 2),
    )
    return list(M)
                ...
                for c in range(newShape[3]):
                    for i in range(newShape[4]):
                        for dim in range(newShape[5]):
                            resampledVectors[x, y, z, c, i, dim, :] = np.interp(
                                config["tFrames"],
                                config["t"],
...
def run(configFile, leapFactor=1, gifWriter="ffmpeg"):
    ...
    if gifWriter == "ffmpeg":
        ...
        raise Exception(
            'Argument gifWriter must be "ffmpeg" or "imagemagick"/"convert"'
        )

    # Set global constants
    global gyro
    gyro = 42577.0  # Gyromagnetic ratio [kHz/T]
        ...
        for y in range(config["ny"]):
            for x in range(config["nx"]):
                for c, component in enumerate(config["components"]):
                    ...
                    vectors[x, y, z, c, :, :, :] = simulateComponent(
        ...
        if output["file"]:
            ...
            if gifWriter == "ffmpeg":
                ...
                if os.path.isdir(tmpdir):
                    rmTmpDir = input(
                        'Temporary folder "{}" already exists. Delete(Y/N)?'.format(
                            tmpdir
                        )
                    )
                    if rmTmpDir.upper() == "Y":
            ...
            for frame in range(0, len(config["tFrames"]), leapFactor):
                ...
                if gifWriter == "ffmpeg":
                    ...
                    filesToSave.append(
                        os.path.join(tmpdir, "{}.png".format(str(frame).zfill(4)))
                    )
            ...
            if gifWriter == "ffmpeg":
                ...
                os.system(
                    (
                        "convert {} -delay {} {}/*png {}".format(
                            compress, delay, tmpdir, outfile
                        )
                    )
                )
                shutil.rmtree(tmpdir)
